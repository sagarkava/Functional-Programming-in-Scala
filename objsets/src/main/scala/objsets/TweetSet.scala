...
class Empty extends TweetSet {
  ...
  override def mostRetweeted: Tweet = throw new scala.NoSuchElementException()
}
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def union(that: TweetSet): TweetSet =
    filterAcc(x => !that.contains(x), that)
    //((right union left) union that) incl elem Doesn't work for big Sets :(

  override def descendingByRetweet: TweetList =
    new Cons(mostRetweeted, (this remove mostRetweeted).descendingByRetweet)

  override def mostRetweeted: Tweet = {
    def max(one: Tweet, two: Tweet): Tweet =
      if (one.retweets > two.retweets) one else two
