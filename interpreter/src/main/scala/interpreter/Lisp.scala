...
object Lisp {
  ...
  def eval1(x: Data, env: Environment): Data = x match {
    ...
    // a better def:
    case 'def :: definition :: body :: rest => definition match {
      case name :: args => eval('def :: name :: List('lambda, args, body) :: rest, env)
    }
    case 'case :: expr_scrut :: firstCase :: restOfCases => {
      firstCase match {
        case 'else :: expression1 :: Nil =>
          eval(expression1, env)
        case value1 :: expression1 :: Nil =>
          eval('if :: List('=, expr_scrut, value1) :: expression1 :: List('case :: expr_scrut :: restOfCases), env)
      }
    }
    case 'quote :: y :: Nil =>
      y
