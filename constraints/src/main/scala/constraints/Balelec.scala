...
object Balelec {
  ...
  def schedule(
    ...
    val varsMatrix: Map[(Task, Volunteer), PropVar] =
      (for (t <- tasks; v <- volunteers) yield (t, v) -> propVar(t.name)).toMap

    // count how many true
    val eachTaskHasEnoughVolunteers: Seq[Formula] = for {
      t: Task <- tasks
      taskToVol: List[PropVar] = volunteers.map(varsMatrix(t, _))
    } yield !positivesLessEquals(taskToVol, t.capacity) && positivesLessEquals(taskToVol, t.capacity + 1)

    // as desirable slots
    val eachVolunteerHasTaskHeCanDo: Seq[Formula] = for {
      t: Task <- tasks
      availabilityCheck: List[Formula] = volunteers.map(v => varsMatrix(t, v) && availability(v).contains(t))
    } yield availabilityCheck.foldLeft[Formula](False)(_ || _)

    // count number of tasks a vol is assigned to
    val maxWorkLoadIsRespected: Seq[Formula] = for {
      v: Volunteer <- volunteers
      taskToVol: List[PropVar] = tasks.map(varsMatrix(_, v))
    } yield positivesLessEquals(taskToVol, maxWorkload)

    val allConstraints: Seq[Formula] = eachTaskHasEnoughVolunteers ++
      eachVolunteerHasTaskHeCanDo ++ maxWorkLoadIsRespected

    val res = solveForSatisfiability(and(allConstraints: _*))

    res.map(model => {
      tasks.map(task => {
        val assignedVolunteers = volunteers.filter(v => model(varsMatrix(task, v)))
